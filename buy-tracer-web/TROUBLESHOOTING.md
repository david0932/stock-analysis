...
### 1. 資料更新失敗：Data.__new__() 參數錯誤（已修正）

**症狀:**
- 更新股票資料時出現錯誤：`Data.__new__() takes 10 positional arguments but 11 were given`
- 快取更新失敗，顯示「更新失敗 (2025-12)」

**原因:**
台灣證券交易所 API 資料格式發生變更，從原本的 9 個欄位增加到 10 個欄位（最後多了一個空字串），但 `twstock` 套件的 `Data` 結構仍為 9 個欄位，導致參數數量不匹配。

**解決方案:**
✅ **系統已建立修補程式自動修復此問題**
- 建立 `utils/twstock_patch.py` 修補檔案
- 應用啟動時自動套用修補程式
- 修補邏輯：只取 API 回傳資料的前 9 個欄位，忽略最後的空字串
- 支援 twstock 1.4.0（最新版）及所有舊版本

**技術細節:**
```python
# API 回傳格式（10 個欄位）
['114/12/01', '122,474,120', '4,796,232,270', '37.05', '40.60',
 '37.00', '40.60', '+3.65', '57,535', '']  # 最後是空字串

# 修補程式只取前 9 個
return DATATUPLE(*data[:9])
```

**無需手動操作** - 修補程式會在應用啟動時自動套用。
...
### 2025-12-26
- ✅ **修復證交所 API 格式變更導致的資料更新錯誤**
  - **問題**: `Data.__new__() takes 10 positional arguments but 11 were given`
  - **根本原因**: 台灣證券交易所 API 資料格式從 9 個欄位變更為 10 個欄位（最後增加一個空字串）
  - **解決方案**: 建立 `utils/twstock_patch.py` 修補檔案
    - 修補 `TWSEFetcher._make_datatuple` 方法，只取前 9 個欄位
    - 修補 `TPEXFetcher._make_datatuple` 方法，只取前 9 個欄位
    - 在應用啟動時自動套用修補程式
  - **影響範圍**: 所有股票的資料更新功能
  - **twstock 版本**: 升級至 1.4.0（最新版）
  - **向後兼容**: 修補程式對所有 twstock 版本都有效
