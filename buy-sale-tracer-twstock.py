...
TICKER_ID = "8110"  # 華東 (8110) - twstock 只需純數字代號
START_DATE_STR = "2025-01-01"  # 設定追蹤開始日期 (格式 YYYY-MM-DD)
...
def generate_signals(df):
    ...
    # 賣點一：MACD 死亡交叉 (動能轉弱)
    macd_dead_cross = (df['DIF'].shift(1) > df['DEM'].shift(1)) & (df['DIF'] < df['DEM'])
    df['Sell_Signal_Type1'] = macd_dead_cross.apply(
        lambda x: "🛑 MACD死亡交叉" if x else "")
    
    # 賣點二：跌破月線 MA20 (中線停損/利)
    # 我們設定為「今日收盤價跌破 MA20 且 MA20 已經開始走平或向下」
    ma20_down_or_flat = (df['MA20'] <= df['MA20'].shift(1))
    break_ma20 = (df['Close'] < df['MA20'])
    df['Sell_Signal_Type2'] = (break_ma20 & ma20_down_or_flat).apply(
        lambda x: "📉 跌破MA20停利/損" if x else "")
...
def plot_signals(df, signals_df):
    ...
    # ------------------- 準備賣點標註 (新增) -------------------
    sell_markers = [(date, row['Close']) for date, row in signals_df.iterrows() if row['Sell_Signal'] != '']
    sell_dates = [date for date, _ in sell_markers]
    
    sell_markers_series = pd.Series(index=df.index, dtype=float)
    for date, price in sell_markers:
        sell_markers_series.loc[date] = price * 1.02 # 向下箭頭位置
        
    signal_scatter_sell = mpf.make_addplot(sell_markers_series, 
                                           type='scatter', 
                                           markersize=100, 
                                           marker='v', # 向下箭頭
                                           color='blue', # 使用藍色區分買點
                                           panel=0)
    add_plots.append(signal_scatter_sell)
    
    # 合併所有 Vlines
    all_vlines = buy_dates + sell_dates
    ...
    mpf.plot(
        ...
        vlines=dict(vlines=all_vlines, colors=['red'], linewidths=[0.5], linestyle='--'), # 使用合併後的 vlines
...
if __name__ == "__main__":
    ...
    if data_df.empty:
        ...
        # 3. 產生訊號
        signal_results = generate_signals(analyzed_df)
        ...
        if not signal_results.empty:
            ...
            print(f"\n--- 🎯 上詮 ({TICKER_ID}) 追蹤買/賣點訊號 ---") # 調整標題
            # 調整輸出列 (新增 Sell_Signal)
            output_columns = ['Close', 'MA20', 'Volume', 'Avg_Volume5', 'DIF', 'DEM', 'OSC', 'Buy_Signal', 'Sell_Signal']
